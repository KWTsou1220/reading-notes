...
系統由多個元件組成，並且元件之間應該獨立實作，不影響其他元件。每個元件提供一個使用者介面，作為對外的溝通。這些元件以**分層**的方式實現是正交化的好方法，每層的元件都只使用到下層提供的介面，避免下層的修改影響到上層的使用。
...
當我們做出了一系列的關鍵決定後，程式碼的靈活性會被縮的越來越小，舉例來說，當我們決定引用 Library A 所提供的函式庫、使用 Company B 的 database...等等，到了開發後期，如果發現 Library A 不夠 efficient、Company B 的 database 速度過慢，此時要替換這些外部引用的資源，是否會造成自身的程式碼大幅度修改？若答案是肯定的，那就代表程式碼的靈活性降低了！

可以增加可逆性的原則：DRY 原則、去耦合、外部設定。去耦合和外部設定將在 Chapter 5 詳細討論。

## 12. 曳光彈

在黑暗中的槍戰，會因為視線不明導致無法命中目標，此時會在彈夾裡填充幾個曳光彈，這種子彈會在黑暗中發光，讓槍手得知目前瞄準的方向正不正確、是否有射擊到敵人。當我們遇到客戶需求不明的時候，可以利用曳光彈開發 (tracer bullet development) 方式進行。最初始階段，我們只實作整個專案的框架，並且利用這個框架把最簡易的功能實作出來。這裡的重點是，我們最初決定的框架會被後續開發延用，因此這個框架需要包含最基本、可以 demo 的功能，這就是我們的曳光彈！

- 使用者可以及早看到成果
- 開發人員建構一個基礎架構，後續的開發流程將會和最初的基本功能開發流程相似
- 可以感受到產品進步

### 曳光彈不會總是擊中目標

最初的功能可能無法符合使用者需求，因此在後續開發過程中，需要對最初的框架適度調整，慢慢瞄準目標。

### 曳光彈程式碼與原型

原型 (prototyping) 是以簡易的方式將 idea 具現化，供 team member、使用者討論、評估難易度，並且進一步發想其他功能。最終開始開發時，會捨棄 prototype，重新建構產品。和曳光彈程式碼不同的是，prototype 是用完即丟的，可以看做是在開發之前進行的偵查與評估。
